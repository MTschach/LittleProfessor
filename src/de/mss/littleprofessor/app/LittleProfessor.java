...
public class LittleProfessor extends JFrame implements WindowListener, MouseListener, ActionListener, KeyListener {
   ...
   protected JPanel getPanelCommons()
   {
      ...
      JLabel l = new JLabel("Klassenstufe");
      l.setBounds(5, 15, 150, 25);
      panelCommons.add(l);
      
      comboSchoolgrade = new JComboBox<>();
      comboSchoolgrade.setBounds(170, 15, 160, 25);
      comboSchoolgrade.addActionListener(this);
      comboSchoolgrade.setActionCommand("changeSchoolgrade");
      
      for (SchoolGrade s : SchoolGrade.values())
         comboSchoolgrade.addItem(s.getName());
      
      panelCommons.add(comboSchoolgrade);
      
      l = new JLabel("Aufgabenzahl");
      l.setBounds(5, 45, 150, 25);
      
      panelCommons.add(l);
      
      textNumberOfTasks = new JTextField(10);
      textNumberOfTasks.setBounds(170, 45, 160, 25);
   ...
   public void actionPerformed(ActionEvent e) {
      ...
      if ("changeSchoolgrade".equals(cmd))
         changeSchoolgrade();
      
      else if ("startTasks".equals(cmd))
         startTasks();
   ...
   private void allTasksDone() {
      ...
      this.resultLabel.setText(getResultText(correctTasks, taskList.size(), duration, SchoolGrade.getByName((String)this.comboSchoolgrade.getSelectedItem())));
   ...
   private String getResultText(int correctTasks, int size, long duration, SchoolGrade schoolgrade) {
   ...
   private void changeSchoolgrade() {
      SchoolGrade sg = SchoolGrade.getByName(comboSchoolgrade.getSelectedItem().toString());
      
      for (PluginInfo pi : loadedPlugins)
      {
         pi.doPreselection(sg);
      }
   }
